...
export default function CustomerContractsPage() {
  ...
  const { data: docsRes, isLoading: isLoadingDocs } = useQuery({
    queryKey: ['documents', 'Customer', id],
    queryFn: () => documentService.getAll({ entity_type: 'Customer', entity_id: id, per_page: 50 }),
    enabled: !!id,
  });
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setExpiryDate('');
      // Invalidate query
      queryClient.invalidateQueries({ queryKey: ['documents', 'Customer', id] });
    } catch (err: any) {
  ...
  return (
    <DashboardLayout 
      active="Customers" 
      title={`Customer Contracts: ${customer?.name || ''}`}
    >
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border border-black/[0.08] rounded-lg p-5 shadow-sm sticky top-24">
              <h3 className="text-sm font-bold text-[#111] mb-4 flex items-center gap-2">
                <UploadCloud size={16} className="text-[#E8450F]" /> Upload Contract
              </h3>
              ...
              <form onSubmit={handleUpload} className="space-y-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    <option value="CustomsClearance">Company Registration</option>
                  </select>
                </div>
                ...
                <Btn 
                  label={isUploading ? 'Uploading...' : 'Upload Contract'} 
                  type="submit" 
            ...
            <div className="bg-white border border-black/[0.08] rounded-lg shadow-sm overflow-hidden min-h-[400px]">
              <div className="p-5 border-b border-black/[0.04]">
                <h3 className="text-sm font-bold text-[#111] flex items-center gap-2">
                  <FileText size={16} className="text-[#E8450F]" /> Uploaded Contracts
                </h3>
              </div>
              
              {isLoadingDocs ? (
                <div className="p-8 text-center text-[#6E6E80] text-sm font-medium">Loading contracts...</div>
              ) : documents.length === 0 ? (
                <div className="p-12 text-center text-[#6E6E80] flex flex-col items-center">
                  ...
                  <p className="text-xs mt-1">Use the form to upload service agreements.</p>
                </div>
              ) : (
