...
| **The single-message gates never re-run per decomposed segment** | **Partially closed 2026-07-30.** Project-selection and site-selection are now gated: `_resolve_project_and_site` (`decomposed_plan.py`) runs once per decomposition (not per segment — project/site membership is a property of the sender, not of any one segment), auto-resolving the single-option case and otherwise holding the already-decomposed segment texts in the new `PendingDecompositionStore` while `render_project_picker`/`render_site_picker` ask, mirroring `_run_project_gate`/`_run_site_gate`'s own hold-and-ask shape. **Still open:** the material-unit-resolution and stock-sufficiency gates remain genuinely per-segment (an ambiguous catalog name or an over-stock usage report is a property of one segment's own fields, not the sender) and are not covered by this pass — a segment hitting either still proceeds with raw extraction fields, surfacing only as a downstream validation failure at execution time. |
