...
| Post-confirmation receipt image (WhatsApp) — after any confirmed record (material, expense, labour, equipment), reply with one visual receipt card instead of plain "✅ Recorded" text. Single shared template across all record types, data-driven, never a different image layout per type. | `apps/whatsapp-assistant/src/channel/receipt/` (new — Jinja2 HTML/CSS re-implementation of the user-supplied `RecordCard` React design, rendered to PNG via Python `playwright`, headless Chromium, in-process — no new service). `channel/whatsapp/outbound.py` gets a new `send_image()`. Wired in at the same point `render_execution_reply()` currently builds the confirm-success text (`interactions/response_handler.py`, consumed by `interactions/handler.py`'s `_resume_and_render()`). | None yet | in progress |
