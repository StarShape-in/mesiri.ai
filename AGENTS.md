...
| Labour & Workforce Module (Web Dashboard) — sidebar navigation category, routes, WhatsApp automations page bound to GET/PATCH /labour/settings, dry-run simulator bound to POST /labour/whatsapp/sandbox/simulate, worker roster, attendance, overview, and settings. | `backend/src/mesiri/domains/workforce/router.py`, `apps/dashboard/src/pages/WhatsAppLabourPage.tsx`, `apps/dashboard/src/components/app-sidebar.tsx`, `apps/dashboard/src/App.tsx`, `apps/dashboard/src/lib/api.ts` | None | done — complete with sidebar category, routes, WhatsApp labour automations page, dry-run sandbox, 271 tests passing, pushed to main |
