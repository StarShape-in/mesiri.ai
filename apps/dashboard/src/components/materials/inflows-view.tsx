...
import { fetchInflows, INFLOW_REASONS, type MaterialReceipt, reverseInflow } from '@/lib/materials'
...
export function InflowsView({ projectId, siteId }: InflowsViewProps) {
  ...
  const reverseMutation = useMutation({
    mutationFn: (id: string) => reverseInflow(id, { reason_note: 'Bulk reversal correction' }),
    onSuccess: () => {
  ...
  const handleBulkReverse = async () => {
    if (confirm(`Are you sure you want to reverse the ${selectedIds.length} selected inflows?`)) {
      try {
        ...
        alert(err.response?.data?.detail || 'Failed to bulk reverse inflows.')
      }
    }
  }
                  ...
                  {items.map((row: MaterialReceipt) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-muted-foreground" onClick={() => openRow(row.id)}>{row.supplier ?? '—'}</TableCell>
                        <TableCell className="text-muted-foreground" onClick={() => openRow(row.id)}>
                          {row.occurred_date}
