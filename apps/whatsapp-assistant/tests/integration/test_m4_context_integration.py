...
async def _seed(conn, text, world: seed.SeedWorld) -> None:
    ...
    for u in world.users:
        await conn.execute(
            text(
                "INSERT INTO context_users (id, is_active, locale, timezone) "
                "VALUES (:i, :a, :l, :t)"
            ),
    ...
    for m in world.memberships:
        await conn.execute(
            text(
                "INSERT INTO organization_memberships (id, organization_id, user_id, is_active) "
                "VALUES (:id, :o, :u, :a)"
            ),
    ...
    for p in world.projects:
        await conn.execute(
            text(
                "INSERT INTO context_projects (id, organization_id, name, is_active) "
                "VALUES (:i, :o, :n, :a)"
            ),
    ...
    for s in world.sites:
        await conn.execute(
            text(
                "INSERT INTO context_sites (id, project_id, organization_id, name, is_active) "
                "VALUES (:i, :p, :o, :n, :a)"
            ),
            {
                "i": s.site_id,
    ...
    for pref in world.preferences:
        await conn.execute(
            ...
            {
                "o": pref.organization_id,
                "u": pref.user_id,
                "dp": pref.default_project_id,
                "ds": pref.default_site_id,
                "l": pref.locale,
                "t": pref.timezone,
            },
        )
...
def _und(fields=None):
    ...
    return UnderstandingResult(
        source_message_id="msg_i",
        correlation_id="cor_i",
...
async def test_project_authorization_and_isolation(engine, redis):
    ...
    ctx = (await r.resolve(_msg(seed.WA_ORGB), _und({"project_name": "Project Alpha"}))).unwrap()
...
async def test_full_resolution_produces_resolved_context(engine, redis):
    ...
    r = ContextResolver(_deps(engine, store))
    ctx = (
        await r.resolve(_msg(seed.WA_ABC_DIRECTOR), _und({"project_name": "Marina Tower"}))
    ).unwrap()
