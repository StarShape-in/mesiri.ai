"""Builds the Project Create workflow graph.

Mirrors workflows/account_admin/graph.py exactly: build_draft has two
outcomes (a real draft, or a clarifying reply with none at all when the
project name wasn't extracted), so this needs the same conditional edge.
...
def build_project_create_graph() -> Any:
    """Compile the Project Create graph:
