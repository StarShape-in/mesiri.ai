...
(the `general_site_update` AI slot) but had no compiled graph --
channel/replies.py's `render_image_purpose_coming_soon` documented this gap
explicitly. This is that graph, for the text/voice path; the photo path
(`img_site_update`) still needs the evidence workflow (plan Phase 4) before
it can attach to an activity, so it is left as-is.
