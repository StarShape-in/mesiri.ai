"""Builds the Automation Setup workflow graph.

Mirrors workflows/site_create/graph.py exactly: build_draft has two outcomes
(a real draft, or a clarifying reply with none at all when time_of_day
wasn't extracted), so this needs the same conditional edge. Compiled
without a checkpointer: WorkflowState.v2 (persisted via the repository
port) is the single durable source of truth. LangGraph is imported lazily
inside this function — the core test suite runs without it installed
...
def build_automation_setup_graph() -> Any:
    """Compile the Automation Setup graph:
