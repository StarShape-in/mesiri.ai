"""Builds the Site Create workflow graph.

Mirrors workflows/project_create/graph.py exactly: build_draft has two
outcomes (a real draft, or a clarifying reply with none at all when the
project wasn't resolved or the site name wasn't extracted), so this needs
the same conditional edge. Compiled without a checkpointer: WorkflowState.v2
...
def build_site_create_graph() -> Any:
    """Compile the Site Create graph:
