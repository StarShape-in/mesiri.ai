"""CreateSiteHandler — unit tests (fakes only, no DB)."""
...
from mesiri.application.projects.fakes import FakeCreateSiteExecutionRepository, FakeDatabase
from mesiri.application.projects.handlers import CreateSiteHandler
...
async def test_valid_create_succeeds_and_persists_one_row():
    repo = FakeCreateSiteExecutionRepository()
    handler = CreateSiteHandler(repo)

    result = await handler.handle(None, _command())

    assert result.status == ExecutionStatus.SUCCEEDED
    assert result.material_row_id is not None
    assert len(repo.site_writes) == 1
...
async def test_invalid_command_is_rejected_without_persisting():
    ...
    assert "site name is required" in result.rejection_reasons
...
async def test_disallowed_role_is_rejected_without_persisting():
    ...
    assert result.status == ExecutionStatus.REJECTED
    assert "only an admin or project manager can create a site" in result.rejection_reasons
    assert repo.site_writes == []


@pytest.mark.asyncio
async def test_missing_project_id_is_rejected_without_persisting():
    repo = FakeCreateSiteExecutionRepository()
    handler = CreateSiteHandler(repo)
    ...
    assert result.status == ExecutionStatus.REJECTED
    assert "a project is required to create a site" in result.rejection_reasons
    assert repo.site_writes == []
...
async def test_handle_confirmed_maps_and_persists_end_to_end():
    repo = FakeCreateSiteExecutionRepository()
    handler = CreateSiteHandler(repo, db=FakeDatabase())

    result = await handler.handle_confirmed(_confirmed({"name": "Block A"}))

    assert result.status == ExecutionStatus.SUCCEEDED
    assert result.idempotency_key == "wf_1"
    assert len(repo.site_writes) == 1
    assert repo.site_writes[0]["command"].project_id == PROJ
...
async def test_handle_confirmed_replays_on_repeat_workflow_instance_id():
    repo = FakeCreateSiteExecutionRepository()
    handler = CreateSiteHandler(repo, db=FakeDatabase())
    confirmed = _confirmed({"name": "Block A"})
