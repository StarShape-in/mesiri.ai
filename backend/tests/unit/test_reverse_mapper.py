...
def test_maps_activity_undo_fields():
    cmd = build_command(_confirmed({"target_kind": "activity", "activity_id": ACTIVITY_ID}))
    assert cmd.target_kind == "activity"
    assert cmd.activity_id == ACTIVITY_ID
    assert cmd.expense_id is None
    assert cmd.money_transaction_id is None
